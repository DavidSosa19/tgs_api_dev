...
public class OperationController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Void>> create(@RequestBody InitOperationRequest operationRequest){
        orchestratorService.initOperation(operationRequest.route(),operationRequest.date());
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok("routes.initialized.success", null));
    }

    @PostMapping("/all")
    public ResponseEntity<ApiResponse<Void>> createAll(@RequestBody InitOperationRequest operationRequest){
